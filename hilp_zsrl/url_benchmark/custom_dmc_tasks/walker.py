...
SUITE = containers.TaggedTasks(allow_overriding_keys=True)
...
def stands(time_limit: int = _DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Stand task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    print("xml imported")
    task = PlanarWalker(move_speed=0, forward=True, random=random)
...
@SUITE.add("benchmarking")
def walks(time_limit: int = _DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Walk task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    print("xml imported")
    task = PlanarWalker(move_speed=_WALK_SPEED, forward=True, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
@SUITE.add("benchmarking")
def runs(time_limit: int = _DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    print("xml imported")
    task = PlanarWalker(move_speed=_RUN_SPEED, forward=True, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
